...
const Registration = () => {
	...
	const { register, handleSubmit, control, getValues, formState: { errors } } = useForm({ mode: 'onChange' })
	...
	return (
		<div
			className={'w-full h-svh select-none bg-[url("/img/auth-background.jpg")] bg-cover bg-no-repeat grid place-content-center'}>
				...
				<form className={'w-full mt-5'} onSubmit={handleSubmit(onSubmit)}>
					<div>
						<label
							...
							className={`normal-case ${errors.email ? 'text-[#fa777c]' : 'text-[#f23f42]'}`}>{errors.email ? '- ' + errors.email.message : '*'}</span></label>
						<input
							className={'outline-0 mt-2 w-full h-10 p-2.5 font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1] mb-5'}
							type="email" id="email" {...register('email', { required: 'Required' })} />
					</div>
					<div>
						...
						<input
							...
							type="text" id="displayName" {...register('displayName')} />
						<div
							className={`tracking-tight text-sm font-medium text-[#dbdee1] duration-300 mt-0 pb-0 h-0 opacity-0 peer-focus:-mt-3 peer-focus:pb-10 peer-focus:opacity-100`}>This
							is how others see you. You can use special characters and emoji.
						</div>
					</div>
					<div>
						<label
							...
							className={`normal-case ${errors.username ? 'text-[#fa777c]' : 'text-[#f23f42]'}`}>{errors.username ? '- ' + errors.username.message : '*'}</span></label>
						<input
							className={'outline-0 mt-2 w-full h-10 p-2.5 font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1] mb-5 peer'}
						...
						/>
						<div
							className={`tracking-tight text-sm font-medium text-[#dbdee1] duration-300 mt-0 pb-0 h-0 opacity-0 peer-focus:-mt-3 peer-focus:pb-10 peer-focus:opacity-100`}>Please
							only use numbers, letters, underscores _, or periods.
						</div>
					</div>
					<div>
						<label
							...
							className={`normal-case ${errors.password ? 'text-[#fa777c]' : 'text-[#f23f42]'}`}>{errors.password ? '- ' + errors.password.message : '*'}</span></label>
						<input
							className={'outline-0 mt-2 w-full h-10 p-2.5 font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1] mb-5'}
					...
					<div>
						<label
							...
							htmlFor="dateOfBirth">date of birth <span className={`normal-case ${errors.birthdayMonth || errors.birthdayDay || errors.birthdayYear ? 'text-[#fa777c]' : 'text-[#f23f42]'}`}>{errors.birthdayMonth || errors.birthdayDay || errors.birthdayYear ? '- ' + (errors.birthdayMonth?.message || errors.birthdayDay?.message || errors.birthdayYear?.message) : '*'}</span></label>
					...
					<Checkbox classes={'mt-4'} id={'offers'}
					...
					<Checkbox classes={'mt-4'} id={'terms'}
							  label={<p>By registering, you agree to Discord's <Link to={'/'}>Terms of
								  Service</Link> and <Link to={'/'}>Privacy Policy</Link>.</p>}
					/>
					<p className={'have-acc mt-5 text-sm font-medium text-[#949ba4] '}><Link to={'/login'}>Already have
						an account?</Link></p>
				</form>
			</main>
		</div>
