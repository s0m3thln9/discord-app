				...
				<form className={'w-full mt-5'} onSubmit={handleSubmit(onSubmit)}>
					<div>
						<label className={'text-xs font-bold uppercase tracking-wide text-[#b5bac1]'}
							...
							className={'normal-case text-[#f23f42]'}>{errors.email ? '- ' + errors.email.message : '*'}</span></label>
						<input
					...
					<div>
						...
						<p className={'text-sm text-[#dbdee1] transition-[.3s] mt-0 pb-0 h-0 opacity-0'}>This is how
							others see you. You can use special characters and emoji.</p>
					</div>
					<div>
						<label className={'text-xs font-bold uppercase tracking-wide text-[#b5bac1]'}
							   $error={errors.userName}
							   htmlFor="userName">username <span
							className={'normal-case text-[#f23f42]'}>{errors.userName ? '- ' + errors.userName.message : '*'}</span></label>
						<input
							className={'outline-0 mt-2 w-full h-10 p-2.5 font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1] mb-5'}
							type="text" id="userName" {...register('userName', { required: 'Required' })}
							onFocus={() => document.querySelector('.help').classList.add('focus')}
							onBlur={() => document.querySelector('.help').classList.remove('focus')} />
						<p className={'text-sm text-[#dbdee1] transition-[.3s] mt-0 pb-0 h-0 opacity-0'}>Please only use
							numbers, letters, underscores _, or periods.</p>
					</div>
					<div>
						<label className={'text-xs font-bold uppercase tracking-wide text-[#b5bac1]'}
							   $error={errors.password}
							   htmlFor="password">password <span
							className={'normal-case text-[#f23f42]'}>{errors.password ? '- ' + errors.password.message : '*'}</span></label>
						<input
							className={'outline-0 mt-2 w-full h-10 p-2.5 font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1] mb-5'}
							type="password" id="password" {...register('password', { required: 'Required' })} />
					</div>
					<div>
						<label className={'text-xs font-bold uppercase tracking-wide text-[#b5bac1]'}
