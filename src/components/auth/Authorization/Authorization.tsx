...
const Authorization = () => {
	...
	return (
		<div
			className={
				'w-full h-svh bg-[url("/img/auth-background.jpg")] bg-no-repeat bg-cover grid place-content-center'
			}
		>
			<main
				className={
					'w-[49rem] gap-16 rounded-[0.3125rem] shadow-div bg-[#313338] p-8 grid grid-cols-[1.725fr_1fr] justify-between max-lg:grid-cols-[100%] max-lg:w-[30rem] max-sm:py-5 max-sm:px-4 max-sm:w-[100vw] max-sm:h-svh max-sm:rounded-none'
				}
			>
				<div className={'grid justify-items-center max-sm:content-start max-sm:w-auto max-sm:h-full'}>
					...
					<form className={'w-full mt-5'} onSubmit={handleSubmit}>
						...
						<input
							className={
								'outline-0 mt-2 w-full h-10 p-[0.625rem] font-medium rounded-[0.1875rem] bg-[#1e1f22] text-[#dbdee1]'
							}
							value={data.email}
							label={'EMAIL'}
							required={true}
